...
def attrib(
    ...
    :param default: A value that is used if an ``attrs``-generated ``__init__.py``
    ...
    :param validator: `callable` that is called by ``attrs``-generated
        ``__init__.py`` methods after the instance has been initialized.  They
    ...
    :param bool init: Include this attribute in the generated ``__init__.py``
        method.  It is possible to set this to ``False`` and set a default
        value.  In that case this attributed is unconditionally initialized
        with the specified default value or factory.
    :param callable converter: `callable` that is called by
        ``attrs``-generated ``__init__.py`` methods to convert attribute's value
    ...
    :param kw_only: Make this attribute keyword-only (Python 3+)
        in the generated ``__init__.py`` (if ``init`` is ``False``, this
...
def _transform_attrs(
    ...
    *collect_by_mro* is True, collect them in the correct MRO order, otherwise
    ...
    # the __init__.py signature and when they aren't kw_only (which are moved to
...
class _ClassBuilder:
    ...
    def add_init(self):
        self._cls_dict["__init__.py"] = self._add_method_dunders(
            _make_init(
                self._cls,
                self._attrs,
                self._has_pre_init,
...
def attrs(
    ...
    :param Optional[bool] hash: If ``None`` (default), the ``__hash__`` method
        ...
        details.
    :param bool init: Create a ``__init__.py`` method that initializes the
        ...
        injected instead. This allows you to define a custom ``__init__.py``
        method that can do pre-init work such as ``super().__init__.py()``,
    ...
    :param bool frozen: Make instances immutable after initialization.  If
        ...
        .. note::
            ...
            3. This *does* have a minor a runtime performance `impact
               ...
               ``__init__.py`` is slightly slower with ``frozen=True``.

            4. If a class is frozen, you cannot modify ``self`` in
               ``__attrs_post_init__`` or a self-written ``__init__.py``. You can
    ...
    :param bool auto_exc: If the class subclasses `BaseException`
        ...
        - all attributes that are either passed into ``__init__.py`` or have a
    ...
    :param bool match_args:
        ...
        non-keyword-only ``__init__.py`` parameter names on Python 3.10 and later.
    ...
    def wrap(cls):
        ...
        if _determine_whether_to_implement(
            cls, init, auto_detect, ("__init__.py",)
        ):
            builder.add_init()
        else:
...
def _make_init(
    ...
    if cls.__module__ in sys.modules:
        # This makes typing.get_type_hints(CLS.__init__.py) resolve string types.
        globs.update(sys.modules[cls.__module__].__dict__)

    globs.update({"NOTHING": NOTHING, "attr_dict": attr_dict})

    if needs_cached_setattr:
        # Save the lookup overhead in __init__.py if we need to circumvent
        # setattr hooks.
        globs["_setattr"] = _obj_setattr

    init = _make_method(
        "__attrs_init__" if attrs_init else "__init__.py",
        script,
        unique_filename,
        globs,
    )
...
def _attrs_to_init_script(
    ...
    # Injecting this into __init__.py globals lets us avoid lookups.
    names_for_globals = {}
    ...
    # For exceptions we rely on BaseException.__init__.py for proper
    # initialization.
    if is_exc:
        vals = ",".join("self." + a.name for a in attrs if a.init)

        lines.append("BaseException.__init__.py(self, %s)" % (vals,))
...
def {init_name}(self, {args}):
    {lines}
""".format(
            init_name=("__attrs_init__" if attrs_init else "__init__.py"),
            args=args,
...
def make_class(name, attrs, bases=(object,), **attributes_arguments):
    ...
    user_init = cls_dict.pop("__init__.py", None)
