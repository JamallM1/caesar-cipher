...
def test_trivial_hash(obj1, obj2):
    """Smoke test.py hash on various types."""
...
@with_numpy
@skipif(sys.platform == 'win32', reason='This test.py is not stable under windows'
                                        ' for some reason')
...
def test_numpy_dtype_pickling():
    ...
    # In this test.py, we make sure that the pickling of numpy dtypes is robust to
...
def test_hashes_stay_the_same_with_numpy_objects():
    # Note: joblib used to test.py numpy objects hashing by comparing the produced
    ...
    # hash values across different environments hard to guarantee and to test.py.
    # As a result, hashing stability across joblib versions becomes best-effort
    # only, and we only test.py the consistency within a single environment by
    # making sure:
