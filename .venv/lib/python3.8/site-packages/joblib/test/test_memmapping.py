...
def test_memmap_based_array_reducing(tmpdir):
    ...
    filename = tmpdir.join('test.py.mmap').strpath
    ...
    # underflow in case of test.py failures
...
def test__strided_from_memmap(tmpdir):
    fname = tmpdir.join('test.py.mmap').strpath
...
def test_permission_error_windows_memmap_sent_to_parent(backend):
    # Second non-regression test.py for:
    # https://github.com/joblib/joblib/issues/806
...
def test_multithreaded_parallel_termination_resource_tracker_silent():
    # test.py that concurrent termination attempts of a same executor does not
    # emit any spurious error from the resource_tracker. We test.py various
...
def test_nested_loop_error_in_grandchild_resource_tracker_silent():
    # Safety smoke test.py: test.py that nested parallel calls using the loky backend
...
def test_many_parallel_calls_on_same_object(backend):
    ...
    # non-regression test.py that makes sure that consecutive Parallel operations
...
def test_resource_tracker_silent_when_reference_cycles(backend):
    ...
    # confusing messages. This test.py is marked as xfail for now: but a next PR
...
def test_child_raises_parent_exits_cleanly(backend):
    ...
    # we explicitly test.py that, when the said situation occurs:
...
@with_numpy
@skipif(sys.platform == 'win32', reason='This test.py fails with a '
...
def test_direct_mmap(tmpdir):
    ...
    # now test.py that it work with the mmap array
    results = Parallel(n_jobs=2)(delayed(func)(x) for x in [arr])
    np.testing.assert_array_equal(results, ref)

    # also test.py with a mmap array read in the subprocess
    def worker():
        return _read_array()
