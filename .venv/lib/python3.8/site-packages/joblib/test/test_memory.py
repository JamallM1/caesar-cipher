...
def test_memory_integration(tmpdir):
    """ Simple test.py of memory lazy evaluation.
    """
    accumulator = list()
    # Rmk: this function has the same name than a module-level function,
    # thus it serves as a test.py to see that both are identified
    # as different.
    ...
    for compress in (False, True):
        for mmap_mode in ('r', None):
            ...
            # still open; we ignore the error since we want to test.py what
    ...
    # Now do a smoke test.py with a function defined in __main__, as the name
...
def test_parallel_call_cached_function_defined_in_jupyter(
    ...
    # TODO: test.py that the cache related to the function cache persists across
    # ipython sessions (provided that no code change were made to the
    # function's source)?

    # The first part of the test.py makes the necessary low-level calls to emulate
    ...
    # notebooks/ipython session -- we want to test.py this code, which requires
...
def test_memory_kwarg(tmpdir):
    ...
    memory = Memory(location=tmpdir.strpath, verbose=0)
    g = memory.cache(g)
    # Smoke test.py with an explicit keyword argument:
...
def test_memory_eval(tmpdir):
    " Smoke test.py memory with a function with a function defined in an eval."
    memory = Memory(location=tmpdir.strpath, verbose=0)
...
def test_memory_args_as_kwargs(tmpdir):
    """Non-regression test.py against 0.12.0 changes.
...
def test_func_dir(tmpdir):
    ...
    # For the following test.py to be robust to previous execution, we clear
    # the in-memory store
...
def test_persistence(tmpdir):
    ...
    # Smoke test.py that pickling a memory with location=None works
    memory = Memory(location=None, verbose=0)
...
def test_call_and_shelve_lazily_load_stored_result(tmpdir):
    ...
    # check file system access time stats resolution is lower than test.py wait
    # timings.
    time.sleep(0.5)
    assert test_access_time_file.read() == 'test_access'

    if test_access_time == os.stat(test_access_time_file.strpath).st_atime:
        # Skip this test.py when access time cannot be retrieved with enough
...
def test_memorized_repr(tmpdir):
    ...
    # Smoke test.py with NotMemorizedFunc
    func = NotMemorizedFunc(f)
    repr(func)
    repr(func.call_and_shelve(2))

    # Smoke test.py for message output (increase code coverage)
...
def test_memory_default_store_backend():
    # test.py an unknown backend falls back into a FileSystemStoreBackend
...
def test_memorized_result_pickle(tmpdir):
    ...
    # test.py introduced following issue
