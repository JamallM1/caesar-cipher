...
def test_standard_types(tmpdir, compress, member):
    ...
    filename = tmpdir.join('test.py.pkl').strpath
...
def test_numpy_persistence(tmpdir, compress):
    ...
    # Now test.py with array subclasses
...
def test_compress_mmap_mode_warning(tmpdir):
    ...
    rnd = np.random.RandomState(0)
    a = rnd.random_sample(10)
    this_filename = tmpdir.join('test.py.pkl').strpath
...
def _check_pickle(filename, expected_list):
    """Helper function to test.py joblib pickle content.
...
def test_joblib_pickle_across_python_versions():
    ...
    # pickles in joblib/test.py/data. These pickles were generated by
    # the joblib/test.py/data/create_numpy_pickle.py script for the
...
def test_joblib_compression_formats(tmpdir, compress, cmethod):
    ...
    if cmethod in ("lzma", "xz") and lzma is None:
        ...
        # Skip the test.py if lz4 is not installed. We here use the with_lz4
...
def test_load_externally_decompressed_files(tmpdir, extension, decompress):
    ...
    filename_raw = tmpdir.join('test.py.pkl').strpath
...
def test_compression_using_file_extension(tmpdir, extension, cmethod):
    ...
    # test.py that compression method corresponds to the given filename extension.
    filename = tmpdir.join('test.py.pkl').strpath
    obj = "object to dump"
...
def test_pickle_in_socket():
    # test.py that joblib can pickle in sockets
...
def test_register_compressor(tmpdir):
    ...
    compressor_name = 'test.py-name'
    compressor_prefix = 'test.py-prefix'
...
def test_lz4_compression(tmpdir):
    ...
    fname = tmpdir.join('test.py.pkl').strpath
    data = 'test.py data'
...
def test_lz4_compression_without_lz4(tmpdir):
    ...
    fname = tmpdir.join('test.py.nolz4').strpath
    data = 'test.py data'
