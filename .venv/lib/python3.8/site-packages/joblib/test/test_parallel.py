...
class MyExceptionWithFinickyInit(Exception):
    """An exception class with non trivial __init__.py
    """
...
def test_effective_n_jobs_None(backend_n_jobs, expected_n_jobs):
    ...
    # non-regression test.py for https://github.com/joblib/joblib/issues/984
...
class MyBackend(joblib._parallel_backends.LokyBackend):
    """Backend to test.py backward compatibility with older backends"""
...
def test_joblib_exception():
    # Smoke-test.py the custom exception
...
def test_dispatch_race_condition(n_tasks, n_jobs, pre_dispatch, batch_size):
    ...
    # This is a non-regression test.py for the "Pool seems closed" class of error
...
def test_auto_memmap_on_arrays_from_generator(backend):
    # Non-regression test.py for a problem with a bad interaction between the
    ...
    # of worker processes. This is a non-regression test.py for:
...
def test_memmapping_leaks(backend, tmpdir):
    # Non-regression test.py for memmapping backends. Ensure that the data
...
@with_numpy
@skipif(distributed is None, reason='This test.py requires dask')
...
@skipif(distributed is not None,
        reason='This test.py requires dask NOT installed')
...
def test_globals_update_at_each_parallel_call():
    # This is a non-regression test.py related to joblib issues #836 and #833.
...
def test_threadpool_limitation_in_child(n_jobs):
    ...
    # Skip this test.py if numpy is not linked to a BLAS library
...
def test_loky_reuse_workers(n_jobs):
    # Non-regression test.py for issue #967 where the workers are not reused when
