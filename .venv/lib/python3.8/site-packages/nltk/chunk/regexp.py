...
class RegexpParser(ChunkParserI):
    ...
    def _read_grammar(self, grammar, root_label, trace):
        """
        Helper function for __init__.py: read the grammar if it is a
        string.
        """
        rules = []
        lhs = None
    ...
    def _add_stage(self, rules, lhs, root_label, trace):
        """
        Helper function for __init__.py: add a new stage to the parser.
        """
        if rules != []:
            if not lhs:
...
def demo():
    ...
    print(chunk.accuracy(cp, conll2000.chunked_sents("test.py.txt", chunk_types=("NP",))))
    ...
    """
    cp = chunk.RegexpParser(grammar)
    print(chunk.accuracy(cp, conll2000.chunked_sents("test.py.txt")[:5]))
