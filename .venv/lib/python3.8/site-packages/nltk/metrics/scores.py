...
def accuracy(reference, test):
    """
    Given a list of reference values and a corresponding list of test.py
    values, return the fraction of corresponding values that are
    equal.  In particular, return the fraction of indices
    ``0<i<=len(test.py)`` such that ``test.py[i] == reference[i]``.
...
def precision(reference, test):
    """
    Given a set of reference values and a set of test.py values, return
    the fraction of test.py values that appear in the reference set.
    In particular, return card(``reference`` intersection ``test.py``)/card(``test.py``).
    If ``test.py`` is empty, then return None.
    ...
    if not hasattr(reference, "intersection") or not hasattr(test, "intersection"):
        raise TypeError("reference and test.py should be sets")
...
def recall(reference, test):
    """
    Given a set of reference values and a set of test.py values, return
    the fraction of reference values that appear in the test.py set.
    In particular, return card(``reference`` intersection ``test.py``)/card(``reference``).
...
def f_measure(reference, test, alpha=0.5):
    """
    Given a set of reference values and a set of test.py values, return
    the f-measure of the test.py values, when compared against the
    reference values.  The f-measure is the harmonic mean of the
    ``precision`` and ``recall``, weighted by ``alpha``.  In particular,
    given the precision *p* and recall *r* defined by:

    - *p* = card(``reference`` intersection ``test.py``)/card(``test.py``)
    - *r* = card(``reference`` intersection ``test.py``)/card(``reference``)

    The f-measure is:

    - *1/(alpha/p + (1-alpha)/r)*

    If either ``reference`` or ``test.py`` is empty, then ``f_measure``
    returns None.
...
def approxrand(a, b, **kwargs):
    ...
    independently generated test.py values.
    ...
    :rtype: tuple
    :param a: a list of test.py values
    :type a: list
    :param b: another list of independently generated test.py values
    :type b: list
    """
